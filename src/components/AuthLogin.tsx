...
const AuthLogin = ({ type }: AuthLoginProps) => {
  ...
  const handleNavigation = () => {
    navigate(type === "login" ? { pathname: "/(auth)/register" } : { pathname: "/(auth)/login" });
  };
  ...
  return (
    <View className="mt-16 space-y-6">
      ...
      <View className="flex-row items-center mb-4">
        ...
        <AppText className="mx-4 text-gray-500 text-sm">{t("auth.social.divider")}</AppText>
        <View className="flex-1 h-px bg-gray-300 dark:bg-gray-600" />
      </View>
      ...
      <Pressable onPress={handleNavigation} className="items-center">
        <AppText className="text-center text-gray-600 dark:text-gray-400">
          {type === "login" ? t("auth.login.noAccount") : t("auth.register.haveAccount")}
          <AppText weight="bold" className="text-[#00C5A7]">
            {type === "login" ? t("auth.login.signUp") : t("auth.register.signIn")}
          </AppText>
        </AppText>
      </Pressable>
    </View>
